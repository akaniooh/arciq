...
// ─── Contract address badge (click to copy) ───────────────────────────────
const CONTRACT_ADDRESS = "0xa038a6fc6401b31438d34aed3261c57f012db2e6";
...
function ContractAddressBadge() {
  ...
  return (
    <button
      onClick={handleCopy}
      className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full border border-[var(--border-default)] bg-white/3 text-[12px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-all"
      aria-label="Copy LENDIQ contract address"
    >
      <span className="font-semibold text-[var(--text-primary)]">$LENDIQ</span>
      <span className="w-1 h-1 rounded-full bg-[var(--text-muted)]" />
      <span className="font-mono">{short}</span>
      {copied ? (
        <svg className="w-3.5 h-3.5 text-[var(--yes-color)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      {copied && <span className="text-[var(--yes-color)]">Copied</span>}
    </button>
  );
}
